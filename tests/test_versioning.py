...
from logging_strict.version_semantic import (
    _map_release,  # pyright: ignore[reportPrivateUsage]
)
from logging_strict.version_semantic import (
    Version,
    get_version,
...
class PackageVersioning(unittest.TestCase):
    ...
    def test_advertised_version_and_url(self) -> None:
    ...
    def test_setuptools_scm_version_file(self) -> None:
        ...
        if len(__version_tuple__) == 3:
            ...
            self.assertEqual(version_actual, ver_long)
        else:
            # 0.0.1a1.dev8
            ver = list(__version_tuple__[:3])
            ver_short = ".".join(map(str, ver))
            ver_dev = cast(str, __version_tuple__[3])
    ...
    def test_get_version(self) -> None:
        ...
        for final in finals:
            for args, kwargs, actual in self.vals:
                expect_info, expect_dev = get_version(
                    actual,
                    is_use_final=final,  # type: ignore[arg-type]
                )
                self.assertEqual(kwargs["dev"], expect_dev)
                self.assertEqual(kwargs["serial"], expect_info[-1])
                if not cast(str, kwargs["releaselevel"]):
                    self.assertEqual(len(expect_info[-2]), 0)
                else:
                    self.assertEqual(cast(str, kwargs["releaselevel"]), expect_info[-2])
        ...
        for dev_pre_0 in dev_pres_0:
            expected = sanitize_tag(dev_pre_0[1])
            expect_info, expect_dev = get_version(expected)

            v_0 = Version(expected)

            v_pre = v_0.pre
            v_pre_is = v_0.is_prerelease
            v_dev = v_0.dev
        ...
        for dev_pre_1 in dev_pres_1:
            expected = sanitize_tag(dev_pre_1[1])
            expect_info, expect_dev = get_version(expected)

            v_2 = Version(expected)

            v_pre = v_2.pre
            v_pre_is = v_2.is_prerelease
            v_dev = v_2.dev
            self.assertEqual(expect_dev, v_dev)
        ...
        for dev_pre_2 in dev_pres_2:
            expected_post = dev_pre_2[2]
            expected = sanitize_tag(dev_pre_2[1])
            expect_info, expect_dev = get_version(expected)

            v_3 = Version(expected)

            v_post = v_3.post
            v_post_is = v_3.is_postrelease
            v_pre_is = v_3.is_prerelease
    ...
    def test_readthedocs_package_name(self) -> None:
        ...
        for package_name in testdata_package_name:
            for ver in ver_:
                str_url = readthedocs_url(
                    package_name,
                    ver_=ver,  # type: ignore[arg-type]
                )
